...
`scripts/stage_generator.gd` が `DEFAULT_STAGE_SEED := 1337` から 1 ステージ分の足場、6 個のシジル、ゴール位置を生成します。`scenes/main.tscn` には固定配置の足場や収集物を置かず、`Platforms` と `Collectibles` の空コンテナへ実行時に生成します。
