...
実行ウィンドウとゲーム内部の論理ビューポートは既定で 1920x1080 です。プロジェクト設定、ゲーム開始時のウィンドウ設定、カメラは 1 倍に統一しています。macOS Retina では `DisplayServer.screen_get_scale()` を加味して物理ウィンドウサイズを設定するため、`godot --path .` で起動したケースでも 1920x1080 相当の実ウィンドウとして表示します。
...
赤いシジルをすべて集めるとゲートが開きます。ステージ上の敵と訓練用ターゲットは斧攻撃や銃撃で壊せます。銃撃は FOCUS を 1 消費し、FOCUS は時間経過で最大 3 まで回復します。敵に触れると HP が減り、開始位置へ戻されます。
